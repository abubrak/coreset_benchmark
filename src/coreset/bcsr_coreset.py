...
class BCSRCoreset:
    ...
    def _optimize_weights_kernel(
        ...
        # 计算RBF核矩阵
        gamma = 1.0 / X_flat.shape[1]  # 默认gamma参数
        K = self._compute_rbf_kernel(X_norm, gamma)

        # 计算类别平衡权重
        unique_labels, counts = np.unique(y_np, return_counts=True)
        class_weights = 1.0 / counts
        sample_class_weights = np.array([
            class_weights[np.where(unique_labels == label)[0][0]]
            for label in y_np
        ])

        # 计算多样性权重（基于核矩阵）
        # 权重与样本在特征空间中的代表性成正比
        diversity_scores = K.mean(axis=1)

        # 组合权重
        weights = sample_class_weights * diversity_scores

        # 归一化到单纯形
        weights = self.projection_onto_simplex(weights)
